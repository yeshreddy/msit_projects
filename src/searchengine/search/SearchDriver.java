/**  
 * 
 * Copyright: Copyright (c) 2004 Carnegie Mellon University
 * 
 * This program is part of an implementation for the PARKR project which is 
 * about developing a search engine using efficient Datastructures.
 * 
 * Modified by Mahender K on 12-10-2009
 */ 


package searchengine.search;


import java.util.*;
import java.io.*;
import java.net.ServerSocket;
import java.net.Socket;

import searchengine.dictionary.ObjectIterator;
import searchengine.indexer.Indexer;
import searchengine.indexer.vecnod;



/**
 * The user interface for the index structure.
 *
 * This class provides a main program that allows users to search a web
 * site for keywords.  It essentially uses the index structure generated
 * by WebIndex or ListWebIndex, depending on parameters, to do this.
 *
 * To run this, type the following:
 *
 *    % java SearchDriver indexfile list|custom keyword1 [keyword2] [keyword3] ...
 *
 * where indexfile is a file containing a saved index and list or custom indicates index structure.
 *
 */
public class SearchDriver
{
    @SuppressWarnings({ "unchecked", "unchecked" })
	public static void main(String [] args)
    {
        Vector<String> v=new Vector<String>();
        DataInputStream inn;
        PrintStream out;
        Indexer w = new Indexer("myhash");
        try{
		    FileInputStream indexSource=new FileInputStream("foo.txt");
		    w.restore(indexSource);
		}
		catch(IOException e){
		    System.out.println(e.toString());
		}
     try
     {
    	 ServerSocket serv=new ServerSocket(4020);
         System.out.println(serv.getInetAddress());
	    System.out.println("server started");
    	while(true)
    	{
    		
    	Socket client=serv.accept();
    	inn = new DataInputStream(client.getInputStream());
    	
    	System.out.println("client IP : "+client.getInetAddress());
		String ob=inn.readLine();
		//System.out.println(ob);
		StringTokenizer st=new StringTokenizer(ob," ");
		String perform=st.nextToken();
		while(st.hasMoreTokens())
		{
			String se=st.nextToken();
			v.addElement(se.toLowerCase());
			//System.out.println(st.nextToken());
		}
    	
		
		// Take care to use the right usage of the Index structure
		// hash - Dictionary Structure based on a Hashtable or HashMap from the Java collections 
		// list - Dictionary Structure based on Linked List 
		// myhash - Dictionary Structure based on a Hashtable implemented by the students
		// bst - Dictionary Structure based on a Binary Search Tree implemented by the students
		// avl - Dictionary Structure based on AVL Tree implemented by the students
		
		System.out.println(perform);    
		System.out.println("search request for : "+v.toString());
		
		
		
		//w.disp();
		/*for(int i=2;i<args.length;i++)
		    v.addElement(args[i]);
		*/
		ObjectIterator<vecnod> i= w.retrievePages(new ObjectIterator<String>(v));
		Vector<vecnod> no_dup_vec=new Vector<vecnod>();
		Vector<vecnod> dup_vec=new Vector<vecnod>();
		/*vecnod check=new vecnod();
		
		Vector<vecnod> check_vec=new Vector<vecnod>();
		check_vec=i.returnVec();
		int index=0;
		while(index<check_vec.size())
		{
			check=check_vec.get(index);
			System.out.println(check.url+" "+check.freq);
			index++;
		}*/
		int argcount=v.size();
		v.clear();
		if(i!=null)
		{
			vecnod temp=new vecnod();
			while(i.hasNext())
			{
			temp=i.next();
			//System.out.println(temp.url);
			//System.out.println(temp.url +" "+temp.freq);
			String s=temp.url.toString();
			//System.out.println(s);
			int bc=1;
			for(int j=0;j<s.length();j++)
			{
				if(s.charAt(j)=='/')
					bc++;
			}
			int rank=temp.freq*100*1/(bc-2);
			//System.out.println(temp.url+" fre "+temp.freq+" dep "+(bc-2)+" rank "+rank);
			temp.freq=rank;
			//System.out.println(temp.url+" "+temp.freq);
			
			
			/***********************************************************/
			//			Applying set or concept to the vector
			//
			/***********************************************************/
			if(perform.equalsIgnoreCase("or"))
			{
				//System.out.println("hi");
			int flag=1;
			for(int k=0;k<no_dup_vec.size();k++)
			{
				vecnod no_dup_vecnode=no_dup_vec.get(k);
				if(no_dup_vecnode.url.toString().equals(temp.url.toString()))
				{
					if(no_dup_vecnode.freq<temp.freq)
						no_dup_vecnode.freq=temp.freq;
					flag=0;
					break;
				}
				
			}
			
			if(flag==1)
			no_dup_vec.add(temp);
			}
			else
			dup_vec.add(temp);
			}
			//System.out.println(dup_vec.toString());
			
			/***************************************************/
			//			Applying set AND concept 				
			//
			/***************************************************/
			if(perform.equalsIgnoreCase("and"))
			{
				//System.out.println("hello");
				vecnod temporary=new vecnod();
				for(int cou=0;cou<dup_vec.size();cou++)
				{
					temporary=dup_vec.get(cou);
					vecnod temporary1=new vecnod();
					int count=0;
					for(int cou1=0;cou1<dup_vec.size();cou1++)
					{
						//if(cou==cou1)
							//continue;
						temporary1=dup_vec.get(cou1);
						if(temporary.url.toString().equals(temporary1.url.toString()))
						{
							count++;
							if(count==argcount)
							{
							int flag=1;
							for(int k=0;k<no_dup_vec.size();k++)
							{
								vecnod no_dup_vecnode=no_dup_vec.get(k);
								if(no_dup_vecnode.url.toString().equals(temporary.url.toString()))
								{
									if(no_dup_vecnode.freq<temporary.freq)
										no_dup_vecnode.freq=temporary.freq;
									flag=0;
									break;
								}
								
							}
							if(flag==1)
							no_dup_vec.add(temporary);
							}
						}
					}
						
					
				}
			}
			
			
			/**********************************/
			//Applying insertion sort 
			//
			/***********************************/
			int in,j;
			vecnod tmp;
		      
		      for(j=1;j<no_dup_vec.size();j++)
		      {
		        in=j-1;
		        tmp=no_dup_vec.get(j);
		        while((in>=0)&&(tmp.freq>no_dup_vec.get(in).freq))
		        {
		          no_dup_vec.set((in+1), no_dup_vec.get(in));	
		          
		          in--;
		        }
		        no_dup_vec.set((in+1),tmp);
		       
		       }

		      
			
			String retstr="";
			System.out.println("Search Results");
			int searchcount=no_dup_vec.size();
			for(int k=0;k<no_dup_vec.size();k++)
			{
				vecnod no_dup_vecnode=no_dup_vec.get(k);
				//System.out.println(no_dup_vecnode.url);//+" rank "+no_dup_vecnode.freq);
				retstr+=no_dup_vecnode.url;
				retstr+="\n";
			}
			retstr+="Found Results ";
			retstr+=searchcount;
			System.out.println(searchcount+" results found");
			out = new PrintStream(client.getOutputStream());
			out.print(retstr);
			client.close();
			
		}
		else
		{
			System.out.println("Search complete.  0  hits found.");
		}
    	}
    }
     catch (Exception e) 
     {
			// TODO Auto-generated catch block
			e.printStackTrace();
	 }
    }
};
